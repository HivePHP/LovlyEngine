import Dom from './Dom.3c5c29e1.js';
import Dropdown from './Dropdown.ec98bfa8.js';
...
export default class NotificationBell {
    ...
    connect() {
        ...
        const socket = window.io(this.rt.url, {
            path: this.rt.path,
            transports: ['websocket', 'polling'],
            auth: { token: this.rt.token },
            reconnection: true,
        ...
        socket.on('realtime:event', () => this.refresh());
        socket.on('realtime:ready', () => {});
        socket.on('connect_error', () => {}); // non-fatal: badges only on load
    }
