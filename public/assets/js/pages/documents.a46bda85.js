...
export default class DocumentsPage {
    ...
    async previewDoc(btn) {
        ...
        if (PREVIEWABLE.includes(ext)) {
            ...
            try {
                const data = await Ajax.post(`/api/documents/${encodeURIComponent(id)}/preview`, {}, {
                    method: 'GET',
                ...
                if (data.type === 'text') {
                    this.setPreviewBody(`<pre class="preview-code"><code>${escapeHtml(data.content)}</code></pre>`);
