...
import RealtimeBus from '../core/RealtimeBus.js';
...
export default class MessagesPage {
    constructor(root) {
        ...
        this.forwardOverlay = Dom.qs('[data-forward-overlay]', root);
        this.forwardList = Dom.qs('[data-forward-list]', root);
        this.forwardSearch = Dom.qs('[data-forward-search]', root);
    ...
    bind() {
        ...
        /* Floating bar — forward */
        Dom.qs('[data-forward-selected]', this.root)?.addEventListener('click', () => {
            this.openForwardModal();
        });

        /* Message context menu */
        Dom.qs('[data-ctx-msg-forward]', this.root)?.addEventListener('click', () => {
            if (this.ctxTargetId) {
                this.selectedIds.clear();
                this.selectedIds.add(this.ctxTargetId);
                this.openForwardModal();
            }
            this.hideAllCtx();
        });
        ...
        Dom.qs('[data-ctx-conv-delete]', this.root)?.addEventListener('click', () => {
            if (this.ctxConvId) this.deleteConversation(this.ctxConvId, 'for_me');
            this.hideAllCtx();
        });
        Dom.qs('[data-ctx-conv-delete-all]', this.root)?.addEventListener('click', () => {
            if (this.ctxConvId) this.deleteConversation(this.ctxConvId, 'for_all');
            this.hideAllCtx();
        });

        /* Forward modal */
        Dom.qs('[data-forward-close]', this.root)?.addEventListener('click', () => {
            this.closeForwardModal();
        });
        this.forwardOverlay?.addEventListener('click', (e) => {
            if (e.target === this.forwardOverlay) this.closeForwardModal();
        });
        this.forwardSearch?.addEventListener('input', () => {
            this.filterForwardList();
        });
    ...
    async deleteConversation(otherId, mode) {
        try {
            await Ajax.post(`/api/messages/${encodeURIComponent(otherId)}/delete-conversation`, { mode });
    ...
    openForwardModal() {
        if (this.selectedIds.size === 0) return;
        if (this.forwardOverlay) this.forwardOverlay.hidden = false;
        if (this.forwardSearch) { this.forwardSearch.value = ''; this.forwardSearch.focus(); }
        this.renderForwardList(this.conversations);
    }

    closeForwardModal() {
        if (this.forwardOverlay) this.forwardOverlay.hidden = true;
    }

    filterForwardList() {
        const q = (this.forwardSearch?.value || '').toLowerCase();
        const filtered = q
            ? this.conversations.filter(c => c.other_name.toLowerCase().includes(q))
            : this.conversations;
        this.renderForwardList(filtered);
    }

    renderForwardList(items) {
        if (!this.forwardList) return;
        this.forwardList.textContent = '';

        if (!items.length) {
            const empty = document.createElement('div');
            empty.className = 'messages-modal-empty';
            empty.textContent = 'Нет диалогов';
            this.forwardList.appendChild(empty);
            return;
        }
        ...
        for (const c of items) {
            ...
            btn.type = 'button';
            btn.className = 'messages-modal-item';
            btn.setAttribute('data-forward-to', String(c.other_id));

            const avatar = document.createElement('span');
            avatar.className = 'messages-modal-avatar';
            if (c.other_avatar) {
                ...
                avatar.textContent = (c.other_name || '?').charAt(0).toUpperCase();
            }
            btn.appendChild(avatar);

            const name = document.createElement('span');
            name.className = 'messages-modal-name';
            name.textContent = c.other_name;
            btn.appendChild(name);

            btn.addEventListener('click', () => this.forwardTo(c.other_id));
            this.forwardList.appendChild(btn);
        }
    }

    async forwardTo(otherId) {
        if (this.selectedIds.size === 0) return;
        const msgs = this.getSelectedMessages();
        this.closeForwardModal();
        try {
            for (const m of msgs) {
                await Ajax.post(`/api/messages/${encodeURIComponent(otherId)}/send`, {
                    body: m.body,
                    forwarded_from: m.sender_id,
                    forwarded_by: this.viewerId,
                });
            }
            this.clearSelection();
            await this.refreshConversations();
            this.openThread(otherId);
        } catch (e) { /* silent */ }
    }
    ...
    getSelectedMessages() {
        ...
        for (const id of this.selectedIds) {
            ...
            if (bubble) {
                const textEl = bubble.querySelector('.message-bubble-text');
                const body = textEl ? textEl.textContent.trim() : bubble.textContent.trim();
                msgs.push({ body, sender_id: Number(bubble.getAttribute('data-sender-id')) });
            }
        }
        return msgs;
    }
    ...
    buildBubble(m) {
        ...
        div.setAttribute('data-sender-id', String(m.sender_id));

        if (m.forwarded_from || m.forwarded_by) {
            div.classList.add('message-bubble--forwarded');

            const header = document.createElement('div');
            header.className = 'message-bubble-forward';

            if (m.forwarded_by) {
                const byRow = document.createElement('div');
                byRow.className = 'message-bubble-forward-row';
                const byAvatar = document.createElement('span');
                byAvatar.className = 'message-bubble-forward-avatar';
                if (m.fb_avatar) {
                    const img = document.createElement('img');
                    img.src = m.fb_avatar;
                    img.alt = '';
                    byAvatar.appendChild(img);
                } else {
                    byAvatar.textContent = (m.fb_name || '?').charAt(0).toUpperCase();
                }
                byRow.appendChild(byAvatar);
                const byLabel = document.createElement('span');
                byLabel.className = 'message-bubble-forward-label';
                byLabel.textContent = 'Переслал ' + (m.fb_name || '');
                byRow.appendChild(byLabel);
                header.appendChild(byRow);
            }

            if (m.forwarded_from) {
                const fromRow = document.createElement('div');
                fromRow.className = 'message-bubble-forward-row';
                const fromAvatar = document.createElement('span');
                fromAvatar.className = 'message-bubble-forward-avatar';
                if (m.ff_avatar) {
                    const img = document.createElement('img');
                    img.src = m.ff_avatar;
                    img.alt = '';
                    ...
                    fromAvatar.textContent = (m.ff_name || '?').charAt(0).toUpperCase();
                }
                fromRow.appendChild(fromAvatar);
                const fromLabel = document.createElement('span');
                fromLabel.className = 'message-bubble-forward-label';
                fromLabel.textContent = (m.ff_name || 'Автор');
